...
from robot_lab.tasks.manip_loco.low_level.config.quadruped.go2_arm.agents import Go2ArmFlatPPORunnerCfg, Go2ArmRoughPPORunnerCfg
from robot_lab.tasks.manip_loco.low_level.config.quadruped.Sirius_Piper.agents import CUHKLRLSiriusPiperRoughPPORunnerCfg, CUHKLRLSiriusPiperFlatPPORunnerCfg
...
if TYPE_CHECKING:
    ...
    # from robot_lab.env.manager_env import ManagerBasedRLEnv
    from cfg.command_cfg import UniformPoseCommandCfg
    from cfg.command_cfg import MLUniformPoseCommandCfg
...
class MLUniformPoseCommand(CommandTerm):
    """Command generator for generating pose commands uniformly.
    ...
    def __init__(self, cfg: MLUniformPoseCommandCfg, env: ManagerBasedRLEnv):
        ...
        self.curr_ee_goal = torch.zeros(self.num_envs, 3, device=self.device)
        self.env = env
        if self.cfg.is_QuadrupedManipulator_Flat == True:
            cfg_runner = CUHKLRLSiriusPiperFlatPPORunnerCfg()
            self.num_env_step = cfg_runner.num_steps_per_env
        else:
            cfg_runner = CUHKLRLSiriusPiperRoughPPORunnerCfg()
            self.num_env_step = cfg_runner.num_steps_per_env
    ...
    def _update_metrics(self):
            # 1. 将基座坐标系下的命令转换到仿真世界坐标系
            # combine_frame_transforms 会自动处理平移和旋转，包括 Z 轴的高度变化
            self.pose_command_w[:, :3], self.pose_command_w[:, 3:] = combine_frame_transforms(
                self.robot.data.root_pos_w,
                self.robot.data.root_quat_w,
                self.pose_command_b[:, :3],
            ...
            # ❌ --- 删除或注释掉下面这段代码 ---
            # 原因：我们现在希望 Z 也是跟随基座移动的（相对高度），
            # 如果保留这行，目标高度会被锁死在采样时的世界高度，导致爬坡时目标穿模。
            
            # if self.cfg.is_QuadrupedManipulator or self.cfg.is_QuadrupedManipulator_Play == True:
            #     self.pose_command_w[:, 2] = self.pose_command_w_z[:, 0] 
            # --------------------------------
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.is_QuadrupedManipulator == True:
            count = torch.tensor(self.env.common_step_counter / self.num_env_step / self.cfg.curriculum_coeff)
            self.pose_command_b[env_ids, 0] = (r.uniform_(*self.cfg.ranges_init.pos_x))  * torch.clamp((1 - count), 0, 1) + \
                                              (r.uniform_(*self.cfg.ranges_final.pos_x)) * torch.clamp((count), 0, 1)
            self.pose_command_b[env_ids, 1] = (r.uniform_(*self.cfg.ranges_init.pos_y))  * torch.clamp((1 - count), 0, 1) + \
                                              (r.uniform_(*self.cfg.ranges_final.pos_y)) * torch.clamp((count), 0, 1)
            self.pose_command_b[env_ids, 2] = (r.uniform_(*self.cfg.ranges_init.pos_z)) * torch.clamp((1 - count), 0, 1) + \
                                            (r.uniform_(*self.cfg.ranges_final.pos_z)) * torch.clamp((count), 0, 1)

            # 2. 反向计算 World Z (仅用于 Visualization 或 Metrics，不影响训练核心)
            # World_Z = Base_Z + Robot_Current_Z
            # 注意：这里我们用当前的 root height 来估算。
            self.pose_command_w_z[env_ids, 0] = self.pose_command_b[env_ids, 2] + self.robot.data.root_pos_w[env_ids, 2]         
            for i in range(len(env_ids)):
                length_arm = torch.norm(torch.stack([self.pose_command_b[i, 0], 
                                                     self.pose_command_b[i, 1], 
                                                     self.pose_command_b[i, 2] 
                                                    ])) 
                while((length_arm > 0.8) or (length_arm < 0.1) or (self.pose_command_b[i, 0] < 0.00 )):
                        self.pose_command_b[i, 0] = (r_1.uniform_(*self.cfg.ranges_init.pos_x))  * torch.clamp((1 - count), 0, 1) + \
                                                        (r_1.uniform_(*self.cfg.ranges_final.pos_x)) * torch.clamp((count), 0, 1) 
                        self.pose_command_b[i, 1] = (r_1.uniform_(*self.cfg.ranges_init.pos_y))  * torch.clamp((1 - count), 0, 1) + \
                                                        (r_1.uniform_(*self.cfg.ranges_final.pos_y)) * torch.clamp((count), 0, 1)
                        self.pose_command_b[i, 2] = (r_1.uniform_(*self.cfg.ranges_init.pos_z))  * torch.clamp((1 - count), 0, 1) + \
                                                        (r_1.uniform_(*self.cfg.ranges_final.pos_z)) * torch.clamp((count), 0, 1)
                        # self.pose_command_b[i, 2] = self.pose_command_w_z[i, 0] - self.robot.data.root_pos_w[i, 2]                     
                        length_arm = torch.norm(torch.stack([self.pose_command_b[i, 0], 
                                                        self.pose_command_b[i, 1], 
            ...
            euler_angles[:, 1] = - torch.atan2(delta_z, torch.sqrt(delta_x**2 + delta_y**2)) + \
                                    r.uniform_(*self.cfg.ranges_init.pitch) * torch.clamp((1 - count), 0, 1) + \
                                    r.uniform_(*self.cfg.ranges_final.pitch) * torch.clamp((count), 0, 1)   
