...
class OnPolicyRunnerWithExtractor(OnPolicyRunner):
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.depth_encoder_cfg is None:
            ...
            obs_dict = {
                "observations": torch.zeros(self.env.num_envs, num_obs, device=self.device),
                "privileged_observations": torch.zeros(self.env.num_envs, num_privileged_obs, device=self.device)
            }
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    if isinstance(self.alg.transition.observations, torch.Tensor):
                        self.alg.transition.observations = {
                            "observations": self.alg.transition.observations,
                            "privileged_observations": privileged_obs  # 把这个也加进去！
                        }
