...
```text
2026-06-17_19-54-10 这条 student 仍然只能作为 Mujoco 临时探测。
student-first 修复没有保住后退稳定性，主线已切回 teacher 精修。
当前 teacher refinement 候选：arclab_arcdog_adjustable_leg_highstep_vae_Teacher/2026-06-19_01-42-04/model_86400.pt
不要直接部署这个 teacher；等待后续重新蒸馏出的盲走 student。
```

Mujoco highstep 地形探测文件：

```text
/home/lxq/colcon_ws/src/quadruped_control_ros2/robot_description/arcdog_adjustable_leg_description/xml/scene_terrain.xml
```

旧地形块已注释，并加入多高度高台用于快速难度检查。这只是针对性探测，不代表策略覆盖 Isaac highstep 的完整训练分布。
