...
## 必须按 FSM 分段解释真机视频（2026-07-30）

E7700 真机视频中，机器人在 `policy2` 阶段已经完成整机上台并保持站立；随后用户
主动切换 `Fixed Down`，下压动作使一条后腿被台边挤出。早期复核曾把切换后的画面
误判成上台失败，这是事件边界判断错误，不是 policy 行为结论。

固定规则：

```text
视频判断必须先用 bag/FSM 确定 policy2、Fixed Stand、Fixed Down 和 passive 的顺序。
policy2 完成目标后的人工状态切换，不得倒算成策略失败。
低频缩略图只能定位片段；最终结论必须复核完整时序并接受用户视觉确认。
bag 中的 clamp、延迟和诊断异常必须保留，但没有因果对齐时不能覆盖可见成功事实。
```

本轮同时确认：

```text
真机活动 config 与本机当前 config 不能互相替代；应保存具名、带日期的只读参考。
普通 Git 只保存报告、路径和 SHA，不提交视频、bag、checkpoint 或 W&B 大文件。
```
