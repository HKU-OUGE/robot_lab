...
As of 2026-06-19, the current highstep student is still not final but can be used for temporary sim-to-sim probing:
...
Current training route after this student:

```text
Do not treat this student as the final deployment branch.
Refine the teacher first for backward stability and sane flat-ground box-joint behavior, then distill a new blind student.
```

Current teacher refinement candidate as of the memory update:

```text
logs/rsl_rl/arclab_arcdog_adjustable_leg_highstep_vae_Teacher/2026-06-19_01-42-04/model_86400.pt
```

This is still a teacher checkpoint, not a deployable blind policy.
...
For quick highstep sim-to-sim checks, the adjustable-leg Mujoco terrain file is:

```text
/home/lxq/colcon_ws/src/quadruped_control_ros2/robot_description/arcdog_adjustable_leg_description/xml/scene_terrain.xml
```

During the 2026-06-18 highstep test, the old terrain block was commented and a multi-height high-platform set was added for checking several Isaac-style difficulty levels. Use this only as a targeted deployment probe; it is not equivalent to the full Isaac training distribution.
