...
```text
The 2026-06-17_19-54-10 student remains a temporary Mujoco probe only.
The main route moved back to teacher refinement because the student-first repair did not preserve backward stability.
Current teacher refinement candidate: arclab_arcdog_adjustable_leg_highstep_vae_Teacher/2026-06-19_01-42-04/model_86400.pt
Do not deploy that teacher directly; wait for a new blind student distillation.
```

Mujoco highstep terrain probe:

```text
/home/lxq/colcon_ws/src/quadruped_control_ros2/robot_description/arcdog_adjustable_leg_description/xml/scene_terrain.xml
```

The old terrain block was commented and a multi-height platform set was added for quick difficulty checks. Treat this as a targeted probe, not as proof that the policy covers the full Isaac highstep training distribution.
