#!/usr/bin/env python3
"""One-shot launcher for the immutable B300 diagonal fresh 7400 run.

The launcher records liveness and the two approved clocks.  It never restarts,
resumes, evaluates, gates, or mutates the training child.
"""
...
SPEC = ROOT / "docs/robotlab_memory_zh/library/highstep_b300_diagonal_imitation_fresh_7400_spec_20260719.md"
SPEC_SHA = "7c8ddc84a5653274981da6d4fc118a8d3c02156e6ec5adcc4d43c996085811e7"
...
WANDB_CONFIG_SHA = "e00225c295534049a47ce3f07aff32a93634cdcbf518d277b36787385e51080a"
...
RUN_NAME = "highstep_b300_diagonal_imitation_fresh_7400_student_from_173499"
WANDB_ENTITY = "xinqili551-the-university-of-hong-kong"
WANDB_PROJECT = "isaaclab"
WANDB_GROUP = WORKFLOW
WANDB_RUN_ID = "s1zsp91e"
...
def verify_authority() -> None:
    ...
    if not (
        ...
        and prereg.get("single_variable_control", {}).get("baseline") == 0.0
        and prereg.get("single_variable_control", {}).get("treatment") == 1.0
        ...
        raise RuntimeError("diagonal 7400 authority/dashboard binding mismatch")
...
def output_directory() -> str | None:
    pattern = (
        "arclab_arcdog_adjustable_leg_highstep_b300_diagonal_imitation_fresh_7400_Student/"
        f"*{RUN_NAME}*"
    )
    candidates = [path for path in (ROOT / "logs/rsl_rl").glob(pattern) if path.is_dir()]
        ...
        HANDOFF,
        {
            **payload,
            "next_step": (
                ...
                if alive
                else "user decision required after unexpected exit"
                if status.startswith("unexpected") or status.startswith("wandb_")
                else "user manual play of the completed checkpoints"
            ),
        },
    )
    ...
    try:
        ...
        if return_code == 0 and relative >= TOTAL_UPDATES:
            write_runtime(
                ...
                "completed_7400_pending_user_manual_play",
                child,
            )
            return 0
